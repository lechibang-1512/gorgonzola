    ...
    for (size_t i = 0; i < entries_.size(); i++) {
        ...
        if (parents->size() > 8) {
            ...
            for (StdIntMap::iterator it = parents->begin(); it != parents->end(); ++it) {
                have_other_guard =
                    have_other_guard && (entries_[it->first].propagate_up_at_count > 1);
            }
    ...
    for (int i = static_cast<int>(v.size()) - 1; i >= 0; i--) {
        ...
        case Prefilter::AND: {
            ...
            entry->propagate_up_at_count =
                prefilter->op() == Prefilter::AND ? static_cast<int>(uniq_child.size()) : 1;
...
void PrefilterTree::RegexpsGivenStrings(
    const std::vector<int>& matched_atoms, std::vector<int>* regexps) const {
    regexps->clear();
    if (!compiled_) {
        // Some legacy users of PrefilterTree call Compile() before
        // adding any regexps and expect Compile() to have no effect.
...
void PrefilterTree::PropagateMatch(const std::vector<int>& atom_ids, IntMap* regexps) const {
    ...
    for (IntMap::iterator it = work.begin(); it != work.end(); ++it) {
        ...
        for (StdIntMap::iterator it = entry.parents->begin(); it != entry.parents->end(); ++it) {
            int j = it->first;
