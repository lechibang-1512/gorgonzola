...
public:
    ...
    virtual Info* PostVisit(
        Regexp* re, Info* parent_arg, Info* pre_arg, Info** child_args, int nchild_args);
...
Prefilter::Info* Prefilter::Info::Walker::ShortVisit(Regexp* re, Prefilter::Info* parent_arg) {
    return AnyMatch();
}

// Constructs the Prefilter::Info for the given regular expression.
// Assumes re is simplified.
Prefilter::Info* Prefilter::Info::Walker::PostVisit(Regexp* re, Prefilter::Info* parent_arg,
    Prefilter::Info* pre_arg, Prefilter::Info** child_args, int nchild_args) {
    Prefilter::Info* info;
    switch (re->op()) {
    default:
    case kRegexpRepeat:
...
Prefilter* Prefilter::FromRE2(const RE2* re2) {
    ...
    Regexp* regexp = re2->getEntireRegex();
    if (regexp == NULL)
        return NULL;
