...
public:
    ...
    bool ParseCCCharacter(
        StringPiece* s, Rune* rp, const StringPiece& whole_class, RegexpStatus* status);

    // Parse a character class range into rr.
    // Removes parsed text from s.
    bool ParseCCRange(
        StringPiece* s, RuneRange* rr, const StringPiece& whole_class, RegexpStatus* status);
...
Regexp::ParseState::ParseState(
    ParseFlags flags, const StringPiece& whole_regexp, RegexpStatus* status)
    : flags_(flags), whole_regexp_(whole_regexp), status_(status), stacktop_(NULL), ncap_(0) {
    if (flags_ & Latin1)
        rune_max_ = 0xFF;
    else
        rune_max_ = Runemax;
...
bool Regexp::ParseState::PushRepeatOp(RegexpOp op, const StringPiece& s, bool nongreedy) {
    ...
    if ((stacktop_->op() == kRegexpStar || stacktop_->op() == kRegexpPlus ||
...
int RepetitionWalker::PostVisit(
    Regexp* re, int parent_arg, int pre_arg, int* child_args, int nchild_args) {
    int arg = pre_arg;
...
// Pushes a repetition regexp onto the stack.
// A valid argument for the operator must already be on the stack.
bool Regexp::ParseState::PushRepetition(int min, int max, const StringPiece& s, bool nongreedy) {
    if ((max != -1 && max < min) || min > maximum_repeat_count || max > maximum_repeat_count) {
...
bool Regexp::ParseState::DoVerticalBar() {
    ...
    Regexp* r1;
    Regexp* r2;
    if ((r1 = stacktop_) != NULL && (r2 = r1->down_) != NULL && r2->op() == kVerticalBar) {
        Regexp* r3;
        if ((r3 = r2->down_) != NULL &&
            ...
            if (r3->op() == kRegexpAnyChar &&
                (r1->op() == kRegexpLiteral || r1->op() == kRegexpCharClass ||
            ...
            if (r1->op() == kRegexpAnyChar &&
                (r3->op() == kRegexpLiteral || r3->op() == kRegexpCharClass ||
...
bool Regexp::ParseState::DoRightParen() {
    ...
    Regexp* r1;
    Regexp* r2;
    if ((r1 = stacktop_) == NULL || (r2 = r1->down_) == NULL || r2->op() != kLeftParen) {
...
struct Splice {
    Splice(Regexp* prefix, Regexp** sub, int nsub)
        : prefix(prefix), sub(sub), nsub(nsub), nsuffix(-1) {}
...
struct Frame {
    Frame(Regexp** sub, int nsub) : sub(sub), nsub(nsub), round(0) {}
...
public:
    static void Round1(
        Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices);
    static void Round2(
        Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices);
    static void Round3(
        Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices);
...
void FactorAlternationImpl::Round1(
    Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices) {
...
void FactorAlternationImpl::Round2(
    Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices) {
    ...
    for (int i = 0; i <= nsub; i++) {
        ...
        if (i < nsub) {
            ...
            if (first != NULL &&
                ...
                (first->op() == kRegexpBeginLine || first->op() == kRegexpEndLine ||
                    first->op() == kRegexpWordBoundary || first->op() == kRegexpNoWordBoundary ||
                    first->op() == kRegexpBeginText || first->op() == kRegexpEndText ||
                    first->op() == kRegexpCharClass || first->op() == kRegexpAnyChar ||
                    first->op() == kRegexpAnyByte ||
                    (first->op() == kRegexpRepeat && first->min() == first->max() &&
...
void FactorAlternationImpl::Round3(
    Regexp** sub, int nsub, Regexp::ParseFlags flags, std::vector<Splice>* splices) {
...
BadEscape:
    ...
    status->set_error_arg(StringPiece(begin, static_cast<size_t>(s->data() - begin)));
    return false;
}

// Add a range to the character class, but exclude newline if asked.
// Also handle case folding.
void CharClassBuilder::AddRangeFlags(Rune lo, Rune hi, Regexp::ParseFlags parse_flags) {

    // Take out \n if the flags say so.
    bool cutnl = !(parse_flags & Regexp::ClassNL) || (parse_flags & Regexp::NeverNL);
    if (cutnl && lo <= '\n' && '\n' <= hi) {
        if (lo < '\n')
            AddRangeFlags(lo, '\n' - 1, parse_flags);
        if (hi > '\n')
...
static void AddUGroup(
    CharClassBuilder* cc, const UGroup* g, int sign, Regexp::ParseFlags parse_flags) {
    if (sign == +1) {
...
ParseStatus ParseUnicodeGroup(
    StringPiece* s, Regexp::ParseFlags parse_flags, CharClassBuilder* cc, RegexpStatus* status) {
    ...
    ::icu::UnicodeString ustr =
        ::icu::UnicodeString::fromUTF8(std::string("\\p{") + std::string(name) + std::string("}"));
...
bool Regexp::ParseState::ParseCharClass(StringPiece* s, Regexp** out_re, RegexpStatus* status) {
    ...
    while (!s->empty() && ((*s)[0] != ']' || first)) {
        ...
        if ((*s)[0] == '-' && !first && !(flags_ & PerlX) && (s->size() == 1 || (*s)[1] != ']')) {
            StringPiece t = *s;
            t.remove_prefix(1); // '-'
            Rune r;
...
static bool IsValidCaptureName(const StringPiece& name) {
    ...
    static const CharClass* const cc = []() {
        CharClassBuilder ccb;
        for (StringPiece group : {"Lu", "Ll", "Lt", "Lm", "Lo", "Nl", "Mn", "Mc", "Nd", "Pc"})
            AddUGroup(&ccb, LookupGroup(group, unicode_groups, num_unicode_groups), +1,
                Regexp::NoParseFlags);
...
bool Regexp::ParseState::ParsePerlFlags(StringPiece* s) {
    ...
    if (t.size() > 2 && t[0] == 'P' && t[1] == '<') {
        ...
        s->remove_prefix(static_cast<size_t>(capture.data() + capture.size() - s->data()));
        return true;
    }
...
BadPerlOp:
    ...
    status_->set_error_arg(StringPiece(s->data(), static_cast<size_t>(t.data() - s->data())));
    return false;
}
...
Regexp* Regexp::Parse(const StringPiece& s, ParseFlags global_flags, RegexpStatus* status) {
    ...
    while (!t.empty()) {
        ...
        Rep:
            ...
            if (ps.flags() & PerlX) {
                ...
                if (!lastunary.empty()) {
                    ...
                    status->set_error_arg(StringPiece(
                        lastunary.data(), static_cast<size_t>(t.data() - lastunary.data())));
                    return NULL;
                }
            }
            opstr = StringPiece(opstr.data(), static_cast<size_t>(t.data() - opstr.data()));
        ...
        case '\\': { // Escaped character or Perl sequence.
            ...
            if ((ps.flags() & Regexp::PerlB) && t.size() >= 2 && (t[1] == 'b' || t[1] == 'B')) {
