...
Regexp::Regexp(RegexpOp op, ParseFlags parse_flags)
    : op_(static_cast<uint8_t>(op)), simple_(false),
      parse_flags_(static_cast<uint16_t>(parse_flags)), ref_(1), nsub_(0), down_(NULL) {
    subone_ = NULL;
...
Regexp* Regexp::StarPlusOrQuest(RegexpOp op, Regexp* sub, ParseFlags flags) {
    ...
    if ((sub->op() == kRegexpStar || sub->op() == kRegexpPlus || sub->op() == kRegexpQuest) &&
...
Regexp* Regexp::ConcatOrAlternate(
    ...
    if (nsub > kMaxNsub) {
        ...
        subs[nbigsub - 1] = ConcatOrAlternate(
            op, sub + (nbigsub - 1) * kMaxNsub, nsub - (nbigsub - 1) * kMaxNsub, flags, false);
        return re;
    }
