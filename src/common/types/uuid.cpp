...
bool UUID::fromString(std::string str, uint128_t& result) {
    if (str.empty()) {
        return false;
    }
...
uint128_t UUID::fromString(std::string str) {
    uint128_t result = 0;
...
uint128_t UUID::fromCString(const char* str, uint64_t len) {
    return fromString(std::string(str, len));
}

void UUID::toString(uint128_t input, char* buf) {
...
std::string UUID::toString(uint128_t input) {
...
ku_uuid_t UUID::generateRandomUUID(RandomEngine* engine) {
    ...
    uint128_t result = 0;
    result.high = 0;
    result.high |= ((uint64_t)bytes[0] << 56);
    result.high |= ((uint64_t)bytes[1] << 48);
    result.high |= ((uint64_t)bytes[2] << 40);
    result.high |= ((uint64_t)bytes[3] << 32);
    result.high |= ((uint64_t)bytes[4] << 24);
    result.high |= ((uint64_t)bytes[5] << 16);
    result.high |= ((uint64_t)bytes[6] << 8);
    result.high |= bytes[7];
    result.low = 0;
