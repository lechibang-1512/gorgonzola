...
enum class GORGONZOLA_API IndexConstraintType : uint8_t {
    PRIMARY = 0,              // Primary key index
    SECONDARY_NON_UNIQUE = 1, // Secondary index that is not unique
};

enum class GORGONZOLA_API IndexDefinitionType : uint8_t {
    BUILTIN = 0,
    EXTENSION = 1,
};
...
struct GORGONZOLA_API IndexType {
    std::string typeName;
...
struct GORGONZOLA_API IndexInfo {
    std::string name;
...
struct GORGONZOLA_API IndexStorageInfo {
    IndexStorageInfo() {}
...
public:
    ...
    GORGONZOLA_API void load(main::ClientContext* context, StorageManager* storageManager,
